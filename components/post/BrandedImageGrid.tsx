...
export function BrandedImageGrid({
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
      {images.map((im) => (
        <div key={im.nodeId} className="overflow-hidden rounded-lg border border-border bg-card">
          <a href={im.url} target="_blank" rel="noopener noreferrer" className="group relative block">
            ...
            {im.placeholder && (
              <span className="absolute left-1.5 top-1.5 rounded bg-background/70 px-1.5 py-0.5 text-[10px] text-amber-300">ảnh mẫu</span>
            )}
            <span className="absolute inset-0 hidden items-center justify-center bg-background/40 text-xs text-white group-hover:flex">Xem lớn ↗</span>
          </a>
          {labels[im.nodeId] && (
            <p className="truncate px-2 pt-1.5 text-xs font-medium text-foreground" title={labels[im.nodeId]}>{labels[im.nodeId]}</p>
          )}
          <div className="flex items-center justify-between gap-1 px-2 pb-1.5 pt-1 text-xs">
            ...
            <form action={downloadStory}>
              ...
              <button className="rounded border border-border px-2 py-0.5 text-foreground hover:border-foreground/30">↓ Story</button>
            </form>
          </div>
        </div>
      ))}
