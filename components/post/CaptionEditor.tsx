...
export function CaptionEditor({
  ...
  const ctl = "inline-flex min-h-[32px] min-w-[32px] items-center justify-center rounded border border-border px-2 text-base text-foreground hover:border-foreground/30 disabled:opacity-30";
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        {lines.map((l, i) => {
          ...
          return (
            <div key={l.uid} className="rounded-lg border border-border bg-card p-3">
              <div className="mb-1.5 flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-wide text-muted-foreground">
                <span className="rounded bg-muted px-1.5 py-0.5 text-foreground">{ROLE_LABEL[l.role] ?? l.role}</span>
                {l.nodeLabel && <span>· {l.nodeLabel}</span>}
                {!l.custom && <span className="text-muted-foreground">· {TONE_LABEL[opt?.tone] ?? opt?.tone}</span>}
                ...
                <div className="ml-auto flex items-center gap-1.5">
                  ...
                  <span className="tabular-nums text-muted-foreground">{l.optIndex + 1}/{l.options.length}</span>
              ...
              {isEditing ? (
                <textarea
                  ...
                  rows={3}
                  className="w-full rounded border border-sky-900/60 bg-background p-2 text-sm leading-relaxed text-foreground outline-none focus:border-sky-600"
                />
              ) : (
                <p className="text-sm leading-relaxed text-foreground">{lineText(l)}</p>
              )}
            </div>
          );
        })}
      ...
      {addable.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 rounded-md border border-dashed border-border p-2">
          <span className="text-xs text-muted-foreground">Thêm đoạn:</span>
          <select value={addKey} onChange={(e) => setAddKey(e.target.value)} className="rounded-md border border-border bg-background px-2 py-1 text-xs text-foreground">
            {addable.map((a) => (
              <option key={a.key} value={a.key}>{(ROLE_LABEL[a.role] ?? a.role)}{a.nodeLabel ? ` · ${a.nodeLabel}` : ""}</option>
            ))}
          </select>
          <button type="button" onClick={add} className="rounded-md border border-border px-2 py-1 text-xs text-foreground hover:border-foreground/30">+ Thêm</button>
        </div>
      )}

      {/* Live assembled caption */}
      <div className="whitespace-pre-line rounded-lg border border-sky-900/50 bg-background p-4 text-sm leading-relaxed text-foreground">
        {caption}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <form action={saveCaption} className="ml-auto">
          ...
          <button type="submit" className="rounded-md border border-border px-3 py-2 text-sm text-foreground hover:border-foreground/30">
            Lưu bản này
          </button>
        </form>
      </div>
