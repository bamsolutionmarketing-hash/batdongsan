...
import { getRotation, bumpRotation, saveScript } from "@/lib/repo/scripts";
...
export async function generateScript(userId: string, input: GenerateInput): Promise<ScriptResult> {
  ...
  const [slots, brandingRes, rotationRes] = await Promise.all([
  ...
  const result = assembleScript({
    ...
    slots, agentTone, rotation, seed, today,
  });
