    ...
    <main className="mx-auto flex max-w-3xl flex-col gap-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{node.category} · {node.facts.length} facts</p>
        </div>
        <Link href={`/admin/projects/${params.id}`} className="text-sm text-muted-foreground hover:text-foreground">← Dự án</Link>
      </div>
          ...
          return (
            <div key={b.id} className="rounded-md border border-border bg-card p-3">
              <div className="flex items-center gap-2 text-xs">
                <span className="rounded bg-muted px-2 py-0.5 text-foreground">{b.role} · v{b.variantNo}</span>
                <span className="text-muted-foreground">{b.tone} · min {b.minConfidence}</span>
                {comp.usable ? (
                  <span className="text-emerald-400">● usable</span>
                ) : (
                  <span className="text-red-400" title={comp.reason ?? ""}>● blocked</span>
                )}
                {!b.isEnabled && <span className="text-muted-foreground">(tắt)</span>}
                <div className="ml-auto flex items-center gap-3">
                  <form action={toggleBlock}>
                    ...
                    <button className="text-xs text-muted-foreground hover:text-foreground">{b.isEnabled ? "Tắt" : "Bật"}</button>
                  </form>
              ...
              <p className="mt-2 whitespace-pre-line text-sm text-foreground">{b.text}</p>
              {b.factKeys.length > 0 && (
                <p className="mt-1 text-xs text-muted-foreground">facts: {b.factKeys.join(", ")}</p>
              )}
      ...
      {/* create */}
      <details className="rounded-md border border-border bg-background p-3" open={blocks.length === 0}>
      ...
      <section className="flex flex-col gap-3">
          ...
          {assets.map((a) => (
            <div key={a.id} className="overflow-hidden rounded-md border border-border bg-card">
              ...
              <div className="flex items-center justify-between px-2 py-1 text-xs">
                <span className="text-muted-foreground">{a.safeZone}</span>
        ...
        <form action={uploadAsset} className="flex flex-wrap items-center gap-2 rounded-md border border-border bg-background p-3">
          <input type="hidden" name="node_id" value={node.id} />
          <input type="hidden" name="project_id" value={params.id} />
